...
tyrano.plugin.kag.tag.clearstack = {
    pm: {stack: ""}, start: function (pm) {
        "" == pm.stack ? this.kag.stat.stack = {if: [], call: [], macro: []} : this.kag.stat.stack[pm.stack] = [];
        this.kag.ftag.nextOrder()
    }
};
tyrano.plugin.kag.tag.close = {
    pm: {ask: "true"}, start: function (pm) {
        var that = this;
...
tyrano.plugin.kag.tag.body = {
    vital: [],
    pm: {bgimage: "", bgrepeat: "", bgcolor: "", bgcover: "false", scWidth: "", scHeight: ""},
    start: function (pm) {
        "" != pm.bgcolor && $("body").css("background-color", $.convertColor(pm.bgcolor));
        if ("" != pm.bgimage) if ("transparent" == pm.bgimage) {
...
tyrano.plugin.kag.tag.html = {
    pm: {layer: "", top: 0, left: 0}, start: function (pm) {
...
tyrano.plugin.kag.tag.endhtml = {
    start: function (pm) {
        var tpm = this.kag.stat.map_html.buff_pm, html = this.kag.stat.map_html.buff_html, html_obj = $("<div></div>");
...
tyrano.plugin.kag.tag.emb = {
    vital: ["exp"], pm: {exp: ""}, log_join: "true", start: function (pm) {
        var val = "" + this.kag.embScript(pm.exp);
...
tyrano.plugin.kag.tag.if = {
    vital: ["exp"], pm: {exp: ""}, log_join: "true", start: function (pm) {
        if (this.kag.embScript(pm.exp)) {
            ...
            for (var i = 0; i < 2e3; i++) {
                if (1 == this.kag.ftag.nextOrderWithTag({else: "", elsif: "", endif: ""})) break
            }
            i > 1900 && this.kag.error("В операторе If допущена ошибка.")
        }
    }
};
...
tyrano.plugin.kag.tag.else = {
    pm: {exp: ""}, log_join: "true", start: function (pm) {
...
tyrano.plugin.kag.tag.call = {
    pm: {storage: null, target: null, countpage: !0, auto_next: "yes"}, start: function (pm) {
        var back_pm = {};
...
tyrano.plugin.kag.tag.macro = {
    vital: ["name"], pm: {name: ""}, log_join: "true", start: function (pm) {
        var index = this.kag.ftag.current_order_index, storage = this.kag.stat.current_scenario;
        ...
        i > 1900 && this.kag.error("The macro is not closed")
    }
};
tyrano.plugin.kag.tag.endmacro = {
    log_join: "true", start: function (pm) {
...
tyrano.plugin.kag.tag.savesnap = {
    vital: ["title"], pm: {title: ""}, start: function (pm) {
        var that = this;
...
tyrano.plugin.kag.tag.autosave = {
    vital: [], pm: {title: ""}, start: function (pm) {
        var that = this;
...
tyrano.plugin.kag.tag.ignore = {
    vital: ["exp"], pm: {exp: ""}, start: function (pm) {
        if (this.kag.embScript(pm.exp)) {
            for (var i = 0; i < 2e3; i++) {
                if (1 == this.kag.ftag.nextOrderWithTag({endignore: ""})) break
            }
            i > 1900 && this.kag.error("IGNORE не закрыт")
...
tyrano.plugin.kag.tag.preload = {
    vital: ["storage"], pm: {storage: "", wait: "false"}, start: function (pm) {
        var that = this;
...
tyrano.plugin.kag.tag.cursor = {
    vital: ["storage"], pm: {storage: "default"}, start: function (pm) {
...
tyrano.plugin.kag.tag.sleepgame = {
    vital: [], pm: {storage: "", target: "", next: !0}, start: function (pm) {
        var that = this, title = this.kag.stat.current_save_str;
...
tyrano.plugin.kag.tag.awakegame = {
    vital: [], pm: {variable_over: "true", bgm_over: "true"}, start: function (pm) {
        if (null == this.kag.tmp.sleep_game) this.kag.ftag.nextOrder(); else {
...
tyrano.plugin.kag.tag.plugin = {
    vital: ["name"], pm: {name: "", storage: "init.ks"}, start: function (pm) {
        var name = pm.name;
...
tyrano.plugin.kag.tag.sysview = {
    vital: ["type", "storage"], pm: {type: "", storage: ""}, start: function (pm) {
        var type = pm.type, storage = pm.storage;
...
tyrano.plugin.kag.tag.loadcss = {
    vital: ["file"], pm: {file: ""}, start: function (pm) {
        var file = pm.file,
...
tyrano.plugin.kag.tag.save_img = {
    vital: [], pm: {storage: "", folder: ""}, start: function (pm) {
        pm.storage;
        var folder = "", storage_url = "";
...
tyrano.plugin.kag.tag.pushlog = {
    vital: ["text"], pm: {text: "", join: "false"}, start: function (pm) {
...
tyrano.plugin.kag.tag.apply_local_patch = {
    vital: ["file"], pm: {file: "", reload: "false"}, start: function (pm) {
        var that = this;
...
tyrano.plugin.kag.tag.check_web_patch = {
    vital: ["url"], pm: {url: "", reload: "false"}, start: function (pm) {
        var that = this;
        ...
        parseFloat(this.kag.variable.sf._patch_version) < parseFloat(obj.version) ? $.confirm("Найдено новое обновление, Ver:" + parseFloat(obj.version) + "「" + obj.message + "」<br />Хотите обновить?", (function () {
            alert("Обновление. После завершения игра автоматически закроется.");
            var http = require("http"), fs = require("fs"), file = obj.file, url = $.getDirPath(pm.url) + file;
...
tyrano.plugin.kag.tag.set_resizecall = {
    vital: ["storage"], pm: {storage: "", target: ""}, start: function (pm) {
