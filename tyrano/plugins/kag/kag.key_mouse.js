tyrano.plugin.kag.key_mouse = {
    ...
    init: function () {
        ...
        $(document).on("mousedown", (function (e) {
            that.clearSkip();
            var target = null;
            2 == e.which ? target = that.map_mouse.center : 3 == e.which && (target = that.map_mouse.right);
            "function" == typeof target ? target() : that[target] && that[target]()
        }));
        var mousewheelevent = "onwheel" in document ? "wheel" : "onmousewheel" in document ? "mousewheel" : "DOMMouseScroll";
        $(document).on(mousewheelevent, (function (e) {
            if (that.canShowMenu() && 0 != that.kag.stat.enable_keyconfig && !($(".menu_close").length > 0 && "none" != $(".layer_menu").css("display"))) {
                var target = null;
                "function" == typeof (target = (e.originalEvent.deltaY ? -e.originalEvent.deltaY : e.originalEvent.wheelDelta ? e.originalEvent.wheelDelta : -e.originalEvent.detail) < 0 ? that.map_mouse.wheel_down : that.map_mouse.wheel_up) ? target() : that[target] && that[target]()
            }
        }));
        ...
        if ("pc" != $.userenv()) {
            layer_obj_click.swipe({
                swipe: function (event, direction, distance, duration, fingerCount, fingerData) {
                    ...
                    that.map_ges[swipe_str] && that[that.map_ges[swipe_str].action] && that[that.map_ges[swipe_str].action]();
    ...
    _role: function (role) {
        ...
        if (!("save" != role && "menu" != role && "quicksave" != role && "sleepgame" != role || 1 != this.kag.stat.is_adding_text && 1 != this.kag.stat.is_wait)) return !1;
        switch (role) {
            case"save":
            ...
            case"quickload":
                ...
                break;
            case"auto":
                1 == this.kag.stat.is_auto ? this.kag.ftag.startTag("autostop", {next: "false"}) : this.kag.ftag.startTag("autostart", {});
                break;
            case"sleepgame":
    ...
    clearSkip: function () {
        ...
        1 == this.kag.stat.is_auto && "true" == this.kag.config.autoClickStop && this.kag.ftag.startTag("autostop", {next: "false"});
