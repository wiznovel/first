...
$.checkThreeModel = function (name) {
    ...
    alert("Модель「" + name + "」не определена.")
};
tyrano.plugin.kag.tag["3d_init"] = {
    vital: [],
    pm: {layer: "0", page: "fore", camera: "Perspective", near: "1", far: "5000", next: "true"},
    clock: {},
    start: function (pm) {
        ...
        var j_canvas = $("<canvas id='three' class='three_canvas'></canvas>"),
            sc_width = parseInt(this.kag.config.scWidth), sc_height = parseInt(this.kag.config.scHeight);
        ...
        const scene = new THREE.Scene, camera_mode = pm.camera + "Camera",
    ...
    initEvent: function (three) {
        var that = this, j_canvas = (three.renderer, three.target_layer, three.j_canvas), camera = three.camera,
            scene = three.scene;
        j_canvas.on("click", (function (event) {
            var x = event.clientX, y = event.clientY, mouse = new THREE.Vector2;
    ...
    updateFrame: function () {
        var three = this.kag.tmp.three, camera = three.camera, models = three.models, delta = this.clock.getDelta();
...
tyrano.plugin.kag.tag["3d_model_new"] = {
    ...
    start: function (pm) {
        var three = this.kag.tmp.three, folder = "";
        ...
        if ("gltf" == ext || "glb" == ext) {
            ...
            (new THREE.GLTFLoader).load(storage_url, (data => {
                var gltf = data, model = gltf.scene;
                let pos = $.three_pos(pm.pos), scale = $.three_pos(pm.scale), rot = $.three_pos(pm.rot);
                ...
                if (animations.length > 0) {
                    ...
                    if ("" != pm.motion) for (var i = 0; i < animations.length; i++) {
            ...
            var obj_url = "./data/" + folder + "/" + pm.storage, mtl_url = obj_url.replace(".obj", ".mtl");
        ...
        } else "mmd" == ext || alert("Ошибка：" + ext + "формат файла не поддерживается")
    }
};
...
tyrano.plugin.kag.tag["3d_sprite_new"] = {
    vital: ["name", "storage"],
    pm: {name: "", storage: "", scale: "", pos: "0", rot: "0", tonemap: "false", next: "true", folder: ""},
    start: function (pm) {
        ...
        $("<img />").attr("src", storage_url).on("load", (e => {
            var width = $(e.currentTarget).get(0).width, height = $(e.currentTarget).get(0).height;
            let pos = $.three_pos(pm.pos), rot = $.three_pos(pm.rot);
            model.position.set(pos.x, pos.y, pos.z);
            model.rotation.set(rot.x, rot.y, rot.z);
            if ("" == pm.scale) model.scale.set(1 * parseInt(width), 1 * parseInt(height), 1); else {
...
tyrano.plugin.kag.tag["3d_event"] = {
    vital: ["name"], pm: {name: "", storage: "", target: ""}, start: function (pm) {
        var three = this.kag.tmp.three;
    ...
    start: function (pm) {
        if ("" == pm.height) {
            ...
            $("<img />").attr("src", texture_url).on("load", (e => {
                var width = $(e.currentTarget).get(0).width, tmp = $(e.currentTarget).get(0).height / width;
...
tyrano.plugin.kag.tag.obj_model_new = {
    ...
    start: function (pm) {
        ...
        let material;
        if ("" != pm.texture) if ("BoxGeometry" == pm.type && pm.texture.split(",").length > 1) {
            var arr_texture = pm.texture.split(","), arr_material = [];
            ...
            material = new THREE.MeshStandardMaterial({map: texture, alphaTest: .01, transparent: !0})
        } else material = new THREE.MeshStandardMaterial({color: parseInt(pm.color.toLowerCase())});
...
tyrano.plugin.kag.tag["3d_hide"] = {
    ...
    start: function (pm) {
        if (0 != $.checkThreeModel(pm.name)) {
            var three = this.kag.tmp.three, options = {duration: parseInt(pm.time)},
...
tyrano.plugin.kag.tag["3d_hide_all"] = {
    vital: [], pm: {time: "500", wait: "true"}, start: function (pm) {
        var three = this.kag.tmp.three, options = {duration: parseInt(pm.time)}, models = this.kag.tmp.three.models,
            cnt_fade = 0, fin_fade = 0;
        for (let key in models) if ("camera" != key) {
            cnt_fade++;
...
tyrano.plugin.kag.tag["3d_delete"] = {
    vital: ["name"], pm: {name: ""}, start: function (pm) {
        if (0 != $.checkThreeModel(pm.name)) {
            var three = this.kag.tmp.three, model = this.kag.tmp.three.models[pm.name];
...
tyrano.plugin.kag.tag["3d_delete_all"] = {
    vital: [], pm: {}, start: function (pm) {
        var three = this.kag.tmp.three, models = this.kag.tmp.three.models;
        for (let key in models) if ("camera" != key) {
...
tyrano.plugin.kag.tag["3d_canvas_show"] = {
    vital: [], pm: {time: "1000"}, start: function (pm) {
        var three = this.kag.tmp.three;
        this.kag.tmp.three.stat.canvas_show = !0;
...
tyrano.plugin.kag.tag["3d_anim"] = {
    vital: ["name"],
    pm: {name: "", time: "1000", effect: "linear", pos: "", rot: "", scale: "", lookat: "", wait: "true"},
    start: function (pm) {
        if (0 != $.checkThreeModel(pm.name)) {
            var three = this.kag.tmp.three, options = {duration: parseInt(pm.time), easing: pm.effect}, map_type = {};
            if ("" != pm.pos) if ("camera" == pm.name && "" != pm.lookat) if (three.models[pm.lookat]) {
            ...
            } else map_type.position = $.three_pos(pm.lookat); else map_type.position = $.three_pos(pm.pos);
            "" != pm.rot && (map_type.rotation = $.three_pos(pm.rot));
            "" != pm.scale && (map_type.scale = $.three_pos(pm.scale));
            var cnt_fin = 0, cnt_type = Object.keys(map_type).length;
            for (let key in map_type) {
                var pos = map_type[key], type = key;
...
tyrano.plugin.kag.tag["3d_anim_stop"] = {
    vital: ["name"], pm: {name: "", finish: "true"}, start: function (pm) {
        if (0 != $.checkThreeModel(pm.name)) {
            this.kag.tmp.three;
...
tyrano.plugin.kag.tag["3d_scene"] = {
    ...
    start: function (pm) {
        var three = this.kag.tmp.three, scene = three.scene, renderer = (three.camera, three.renderer);
        if ("" != pm.light_amb) {
...
tyrano.plugin.kag.tag["3d_camera"] = {
    ...
    start: function (pm) {
        var three = this.kag.tmp.three, camera = three.camera;
        three.renderer;
        if ("" != pm.pos) {
            let pos = $.three_pos(pm.pos);
...
tyrano.plugin.kag.tag["3d_gyro"] = {
    vital: [], pm: {max_x: "30", max_y: "30", mode: "rotation", next: "true"}, start: function (pm) {
        var three = this.kag.tmp.three, camera = three.camera;
        three.renderer;
        {
            const GyroMonitor = device_type => {
                var first_beta = 0, first_gamma = 0, first_flag = !0, max_y = parseFloat(pm.max_y),
                    max_x = parseFloat(pm.max_x), default_camera_y = camera.rotation.y,
                    default_camera_x = camera.rotation.x, default_camera_pos_y = camera.position.y,
                    default_camera_pos_x = camera.position.x, angle = 0;
                parseInt(pm.frame);
                ...
                const orientEvent = e => {
                    if (1 == first_flag) {
                        ...
                        if (0 != angle) [max_x, max_y] = [max_y, max_x]; else {
                            max_x = pm.max_x;
                            max_y = pm.max_y
                        }
                    }
                    if (angle == this.kag.tmp.angle) {
                        ...
                        } else if (2 == three.stat.gyro.mode) if (0 == angle) {
                ...
                var sc_width = parseInt(this.kag.config.scWidth), sc_height = parseInt(this.kag.config.scHeight),
                    sc_x = sc_width / 2, sc_y = sc_height / 2;
                const mouseMoveEvent = e => {
                    var x = e.clientX, y = e.clientY, p_x = (x -= sc_x) / sc_x, p_y = (y = -1 * (y - sc_y)) / sc_y,
                        max_x = parseFloat(pm.max_x), max_y = parseFloat(pm.max_y), gyro_x = 0, gyro_y = 0;
                    if (1 == first_flag) {
                        first_flag = !1;
                        "rotation" == pm.mode ? three.stat.gyro.mode = 1 : three.stat.gyro.mode = 2
                    }
    ...
    vital: [], pm: {name: "camera", button_text: "Закрыть отладку камеры", menu: "true"}, start: function (pm) {
        var three = this.kag.tmp.three, j_canvas = three.j_canvas, target_layer = three.target_layer,
            old_target_layer_zindex = target_layer.css("z-index"), old_canvas_zindex = j_canvas.css("z-index");
        j_canvas.css("z-index", 9999999);
        target_layer.css("z-index", 9999999);
        var model = this.kag.tmp.three.models[pm.name].model, renderer = three.renderer,
            mousedown = (three.camera, parseInt(this.kag.config.scWidth), parseInt(this.kag.config.scHeight), !1),
            button = 0, original_v = (new THREE.Vector3, new THREE.Vector3, new THREE.Vector3, $.setVector(model)),
            first_client_x = 0, first_client_y = 0, first_model_x = 0, first_model_y = 0, first_model_z = 0;
        ...
        var j_debug_msg = $("<div style='padding:5px'><input type='text' style='width:320px' /></div>"),
            j_copy_button = $("<input type='button' value='Копировать' />");
        j_copy_button.on("click", (e => {
            evt_mouseup();
            j_debug_msg.find("input").select();
            document.execCommand("copy")
        }));
        var j_reset_button = $("<input type='button' value='Сбросить' />");
...
tyrano.plugin.kag.tag["3d_motion"] = {
    vital: ["name", "motion"], pm: {name: "", motion: ""}, start: function (pm) {
        if (0 != $.checkThreeModel(pm.name)) {
            this.kag.tmp.three;
...
tyrano.plugin.kag.tag["3d_debug"] = {
    vital: ["name"],
    pm: {name: "", button_text: "Закрыть 3D отладку", menu: "true", overlap: "false", reset: "false"},
    start: function (pm) {
        var three = this.kag.tmp.three, j_canvas = three.j_canvas, target_layer = three.target_layer,
            old_target_layer_zindex = target_layer.css("z-index"), old_canvas_zindex = j_canvas.css("z-index"),
            model_obj = this.kag.tmp.three.models[pm.name], model = model_obj.model,
            camera = (three.renderer, three.camera),
            prevPosition = (parseInt(this.kag.config.scWidth), parseInt(this.kag.config.scHeight), {}), mousedown = !1,
            button = 0, vec = new THREE.Vector3, pos = new THREE.Vector3, original_pos = new THREE.Vector3,
            hen_pos = {x: 0, y: 0, z: 0}, original_v = $.setVector(model), first_client_y = 0, first_model_z = 0;
        ...
        function evt_mousedown(e) {
            if (0 == e.button) button = 0; else if (1 == e.button) {
                button = 1;
                first_client_y = e.clientY;
                first_model_z = model.position.z
            } else if (2 == e.button) {
